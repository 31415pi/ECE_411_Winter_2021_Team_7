...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, SPIClass *spixx,
...
void Adafruit_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height()-16)) {
...
void Adafruit_SSD1306::clearDisplay(void) {
  memset(buffer, 0, WIDTH * ((HEIGHT-16 + 7) / 8));
}
...
void Adafruit_SSD1306::drawFastHLineInternal(int16_t x, int16_t y, int16_t w,
  ...
  if ((y >= 0) && (y < HEIGHT-16)) { // Y coord in bounds?
    if (x < 0) {                  // Clip left
      w += x;
      x = 0;
    }
...
void Adafruit_SSD1306::drawFastVLineInternal(int16_t x, int16_t __y,
  ...
  if ((x >= 0) && (x < WIDTH)) { // X coord in bounds?
    ...
    if ((__y + __h) > HEIGHT-16) { // Clip bottom
      __h = (HEIGHT-16 - __y);
    }
...
bool Adafruit_SSD1306::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width() - 64) && (y >= 0) && (y < height()-16)) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch (getRotation()) {
    case 1:
      ssd1306_swap(x, y);
...
void Adafruit_SSD1306::display(void) {
  ...
  uint8_t *line;
  for (int i = 0; i < 48; ++i) { // Send each line twice
    line = &buffer[i * WIDTH/8];
